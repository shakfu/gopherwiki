...
var gopherwiki_editor = {
    ...
    _toggleBlock: function(syntax_chars, token) {
        ...
        if (cm_editor.getSelection().length == 0) {
            ...
            const word = gopherwiki_editor._findWordAt(cm_editor.getCursor());
    ...
    _toggleMultilineBlock: function(syntaxStartChars, headerRegex = null, syntaxEndChars = null, placeholderContent = null) {
        ...
        let selectedLines = gopherwiki_editor._getSelectedLines();
        ...
        if (selectedLines.length == 1 && firstLine.match(/^$/)) { // user selected an empty line
            ...
            gopherwiki_editor._setLine(selectedLines[0], placeholderBlock);
            ...
            gopherwiki_editor._setSelectedLines(selectedLines);
        ...
        if (headerRegex !== null && firstLine.match(headerRegex)) {
            ...
            if (isHeader) {
                gopherwiki_editor._setLine(selectedLines[0], "");
                removeBlock = true;

            } else { // the first line is a header, but a different one -> replace header and quit
                gopherwiki_editor._setLine(selectedLines[0], headerValue);
                cm_editor.focus();
                return;
            }
            ...
            selectedLines = gopherwiki_editor._getSelectedLines();
        ...
        if (removeBlock) {
            ...
            if (!endFound) {
                ...
                let blockEndLine = -1;
                for (const endChar of syntaxEndChars) {
                    blockEndLine = gopherwiki_editor._findNextOccurenceLine(endChar)
                ...
                if (blockEndLine !== -1) {
                    gopherwiki_editor._setLine(blockEndLine, "");
                } else {
            ...
            gopherwiki_editor._setLine(lastLineNum, prefix + tailValue);
    ...
    _toggleLines: function(line_prefix, line_re, token) {
        ...
        linenumbers = gopherwiki_editor._getSelectedLines();
        var count = 1;
        for (const ln of linenumbers) {
            ...
            for (const re of line_re) {
                ...
                gopherwiki_editor._setLine(ln, updated_line);
            }
            count += 1;
        }
        cm_editor.focus();
    ...
    _toggleLinesMultiLevel: function(indentChar, maxLevel=maxMultiLevels) {
        if (!cm_editor) { return; }
        for (const i of gopherwiki_editor._getSelectedLines()) {
            ...
            if (lineHLevel == 0) {
                gopherwiki_editor._setLine(i, indentChar + " " + line);
                ...
                gopherwiki_editor._setLine(i, line.replace(new RegExp("^(?:" + indentChar + ")+\\s*"), ''));
            }
        }
        cm_editor.focus();
    },
    _toggleAlert: function(header, placeholderContent = null) {
        ...
        if (selectedLines.length == 1 && firstLine.match(/^$/)) { // user selected an empty line
            ...
            const firstLineNum = selectedLines[0];
            gopherwiki_editor._setLine(firstLineNum, "\n" + placeholderContent.join("\n"));
        ...
        if (firstLine.match(headerRegex)) {
            ...
            if (firstLine.includes(headerValue)) {
                gopherwiki_editor._setLine(selectedLines[0], "> "); // keep the quote, as this is handled below

            } else { // the first line is a header, but a different one -> replace header and quit
                gopherwiki_editor._setLine(selectedLines[0], firstLine.replace(/\[![A-Z]+]/, headerValue));
                return;
            }
    ...
    header: function() {
        gopherwiki_editor._toggleLinesMultiLevel("#");
    },
    bold: function() {
        gopherwiki_editor._toggleBlock(["**","__"], "bold");
    },
    italic: function() {
        gopherwiki_editor._toggleBlock(["_","*"], "italic");
    },
    strikethrough: function() {
        gopherwiki_editor._toggleBlock("~~", "strikethrough");
    },
    spoiler: function() {
        const placeholder = "Hidden content";
        const selectedLines = gopherwiki_editor._getSelectedLines();
        const firstLine = cm_editor.getLine(selectedLines[0]);
        let placeholderUsed = false;

        if (selectedLines.length == 1 && firstLine.match(/^\s*$/)) {
            gopherwiki_editor._setLine(selectedLines[0], placeholder);
            placeholderUsed = true;
        }
    ...
    code: function() {
        gopherwiki_editor._toggleBlock(["`", "```"], "code");
    },
    codeBlock: function() {
        gopherwiki_editor._toggleMultilineBlock("```", /^```\w*/, null, "code");
    },
    hr: function() {
        let linenumbers = gopherwiki_editor._getSelectedLines();
        let changed = false;
        let line = "";
        let ln = 0;
        ...
        if (!changed) {
            ...
            gopherwiki_editor._setLine(ln, line_insert);
            // update cursor
    ...
    quote: function (multilevel = true) {
        gopherwiki_editor._toggleLinesMultiLevel(">", multilevel ? maxMultiLevels : 1);
    },
    ul: function() {
        gopherwiki_editor._toggleLines("- ",[/\s*[-+*]\s+/], "ul");
   },
    ol: function() {
        gopherwiki_editor._toggleLines("\\d",[/\s*\d+\.\s+/], "ol");
    },
    cl: function() {
        gopherwiki_editor._toggleLines("- [ ] ",[/\s*[-+*] \[ \]\s+/], "ul");
    },
    panelNotice: function() {
        gopherwiki_editor._toggleMultilineBlock(":::info", /^:::(info|warning|danger)/, ":::", ["# Header", "Content"]);
    },
    panelWarning: function() {
        gopherwiki_editor._toggleMultilineBlock(":::warning", /^:::(info|warning|danger)/, ":::", ["# Header", "Content"]);
    },
    panelDanger: function() {
        gopherwiki_editor._toggleMultilineBlock(":::danger", /^:::(info|warning|danger)/, ":::", ["# Header", "Content"]);
    },
    img: function(img = "![]()") {
        if (!cm_editor) { return; }
        state = gopherwiki_editor._getState();
    ...
    diagram: function() {
        gopherwiki_editor._toggleMultilineBlock("```mermaid", /^```mermaid/, "```", "See https://mermaid.js.org/intro/");
    },
    alertNote: function() {
        gopherwiki_editor._toggleAlert("note", ["Content"]);
    },
    alertTip: function() {
        gopherwiki_editor._toggleAlert("tip", ["Content"]);
    },
    alertImportant: function() {
        gopherwiki_editor._toggleAlert("important", ["Content"]);
    },
    alertWarning: function() {
        gopherwiki_editor._toggleAlert("warning", ["Content"]);
    },
    alertCaution: function() {
        gopherwiki_editor._toggleAlert("caution", ["Content"]);
    },
    link: function(text = "description", url = "https://") {
        if (!cm_editor) { return; }
        let state = gopherwiki_editor._getState();
        // we don't mess with existing tokens of these kinds
        ...
        if (cm_editor.getSelection().length == 0) {
            // if nothing is selected, select the word under the cursor
            anythingSelected = false;
            let word = gopherwiki_editor._findWordAt(cm_editor.getCursor());
            cm_editor.setSelection(word.anchor, word.head);
        }
        if (cm_editor.getSelection().trim().length == 0) {
    ...
    footnote: function() {
        ...
        if (selectedLines.length > 1) {
            ...
            gopherwiki_editor._setSelectedLines(selectedLines.slice(selectedLines.length - 1));
            selectedLines = gopherwiki_editor._getSelectedLines();
        }
        ...
        gopherwiki_editor._setLine(selectedLines[0], newLine);
        ...
        gopherwiki_editor._setLine(lastLine, lastLineContent + separator + "\n" + startChar + footNoteValue + endChar + ": footnote_description");
    ...
    _findTable: function() {
        ...
        const block = gopherwiki_editor._findBlock(true);
        // no block found
        if (!block) return;
        ...
        if (relative_cursor.line == 0) relative_cursor.col = gopherwiki_editor._findCursorCell(header, relative_cursor.ch);
        if (relative_cursor.line == 1) relative_cursor.col = gopherwiki_editor._findCursorCell(align, relative_cursor.ch);
        ...
        for (i in celltext)
        {
            ...
            // calc column for relative_cursor
            if (relative_cursor.line - 2 == i) relative_cursor.col = gopherwiki_editor._findCursorCell(line, relative_cursor.ch) - 1;
    ...
    _tableArray: function(t) {
        ...
        for (var j in t.header) {
            row.push(" "+gopherwiki_editor._alignStr(t.header[j], t.colum_width[j], t.align[j])+" ");
        }
        arr.push(row);
        ...
        for (var i in t.cells) {
            ...
            for (var j in t.cells[i]) {
                row.push(" "+gopherwiki_editor._alignStr(t.cells[i][j], t.colum_width[j], t.align[j])+" ");
            }
            arr.push(row);
        }
        return arr;
    ...
    table: function() {
        ...
        t = gopherwiki_editor._findTable();
        if (!(t)) {
            ...
            arr = gopherwiki_editor._tableArray(t);
            gopherwiki_editor.table_arr_replace(arr);
        }
        cm_editor.focus();
    },
    // row manipulation
    table_add_row: function() {
        if (!cm_editor) return;
        // find table
        var t = gopherwiki_editor._findTable();
        if (!t) return;
        // get table as array
        var arr = gopherwiki_editor._tableArray(t);
        // craft row
        var empty_row = [];
        for (var j in t.colum_width)
        {
            empty_row.push(gopherwiki_editor._alignStr("", t.colum_width[j]+2, t.align[j]));
        }
        // insert row into arr
        ...
        // update editor
        gopherwiki_editor.table_arr_replace(arr, Math.max(2, row));
        cm_editor.focus();
    },
    table_remove_row: function() {
        ...
        arr.splice(row, 1);
        // update editor
        gopherwiki_editor.table_arr_replace(arr, row, t.col);
        cm_editor.focus();
    },
    table_move_row_up: function() {
        ...
        if (t.row < 3) {
            ...
            // update editor
            gopherwiki_editor.table_arr_replace(arr, t.row - 1, t.col);
        }
        cm_editor.focus();
    },
    ...
    table_add_column: function() {
        ...
        // update editor
        gopherwiki_editor.table_arr_replace(arr, t.row, t.col);
        cm_editor.focus();
    },
    table_remove_column: function() {
        ...
        // update editor
        gopherwiki_editor.table_arr_replace(arr, t.row, Math.min(arr[0].length-1,t.col));
        cm_editor.focus();
    },
    table_move_column: function(offset = 1) {
        ...
        if (t.col+offset >= arr[0].length || t.col+offset < 0) {
            ...
            // update editor
            gopherwiki_editor.table_arr_replace(arr, t.row, t.col+offset);
        }
        cm_editor.focus();
    },
    ...
    insert_attachment: function() {
        ...
        if (attachment_type == "image") {
            gopherwiki_editor.img('![]('+url+')\n');
        } else if (attachment_type == "thumbnail") {
            gopherwiki_editor.img('[![]('+thumbnail_url+')]('+url+')\n');
        } else { // link
            gopherwiki_editor.img('['+filename+']('+url+')\n');
        }
    },
    ...
    on_paste: function(e) {
        ...
        if (typeof clipboardData === "object") {
            ...
            if (matched) {
                result = gopherwiki_editor.paste_url(pastedText);
            }
        }
...
window.addEventListener("keydown", function(event) {
    ...
    if (inCodeMirror) {
        if (command && !shift && !alt && key === 'b') {
            gopherwiki_editor.bold();
            event.preventDefault();
            return;
        }

        if (command && !shift && !alt && key === 'i') {
            gopherwiki_editor.italic();
            event.preventDefault();
            return;
        }

        if (command && !shift && !alt && key === 'k') {
            gopherwiki_editor.link();
            event.preventDefault();
            return;
        }
        ...
        */
        ...
        if (command && shift && !alt && key === 's') {
            gopherwiki_editor.strikethrough();
            event.preventDefault();
            return;
        }
        ...
        */
        ...
        if (!command && !shift && alt && key === "ArrowUp") {
            gopherwiki_editor.table_add_row();
            event.preventDefault();
            return;
        }
        ...
        if (!command && !shift && alt && key === "ArrowDown") {
            gopherwiki_editor.table_add_row();
            gopherwiki_editor.table_move_row_down();
            event.preventDefault();
            return;
        }
        */
